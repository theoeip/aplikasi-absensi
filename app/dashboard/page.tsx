// Lokasi File: app/dashboard/page.tsx (VERSI DEFINITIF DENGAN PERBAIKAN QUERY)
'use client';
...
const DashboardHeader = ({ userProfile, onLogout }: { userProfile: UserProfile | null, onLogout: () => void }) => ( <header className="bg-purple-700 text-white shadow-md"> <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8"> <div className="flex justify-between items-center py-4"> <div className="flex items-center space-x-4"> <FaGraduationCap size={24} /> <span className="text-xl font-bold">EduAttend - Dashboard Guru</span> </div> <div className="flex items-center space-x-4"> <div className="text-right"> <p className="font-semibold">Selamat datang, {userProfile?.full_name || '...'}</p> <p className="text-sm opacity-90">{userProfile?.role || 'Guru'}</p> </div> <button onClick={onLogout} className="p-2 rounded-full hover:bg-white/20 transition-colors"> <FaSignOutAlt size={20} /> </button> </div> </div> </div> </header> );
const AppMenuCard = () => { const menuItems = [ { icon: BsFillGrid3X3GapFill, title: "Absensi Siswa", desc: "Kelola data siswa", href: "/admin/attendance" }, { icon: IoIosDocument, title: "Data Kelas", desc: "Kelola data kelas", href: "/admin/classes" }, { icon: BsPencilSquare, title: "Ulangan", desc: "Buat & kelola ujian", href: "/admin/exams" }, { icon: BsUpload, title: "Upload Soal", desc: "Upload bank soal", href: "/admin/questions" }, { icon: FaTasks, title: "Tugas", desc: "Kelola tugas siswa", href: "/admin/assignments" }, { icon: BsChatDotsFill, title: "Group Chat", desc: "Chat per kelas", href: "/admin/chat" }, { icon: BsCalendarEvent, title: "Jadwal", desc: "Jadwal mengajar", href: "/admin/schedule" }, { icon: BsBarChartFill, title: "Nilai", desc: "Input & lihat nilai", href: "/admin/grades" }, { icon: IoIosDocument, title: "Laporan", desc: "Generate laporan", href: "/admin/reports" }, ]; return ( <Card className="h-full"> <CardHeader><CardTitle className="flex items-center gap-2"><BsFillGrid3X3GapFill/> Menu Aplikasi</CardTitle></CardHeader> <CardContent><div className="grid grid-cols-2 md:grid-cols-3 gap-4">{menuItems.map(item => (<Link href={item.href} key={item.title} className="p-4 bg-gray-50 hover:bg-gray-100 rounded-lg flex flex-col items-center text-center gap-2 transition-colors"><item.icon size={28} className="text-purple-600"/><p className="font-semibold text-sm">{item.title}</p><p className="text-xs text-gray-500 hidden md:block">{item.desc}</p></Link>))}</div></CardContent> </Card> ); }
const StatsBar = () => { const stats = [ { value: "156", label: "Total Siswa", icon: FaUsers, color: "text-blue-500" }, { value: "8", label: "Kelas Aktif", icon: FaGraduationCap, color: "text-green-500" }, { value: "24", label: "Tugas Pending", icon: FaTasks, color: "text-yellow-500" }, ]; return ( <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{stats.map(stat => (<Card key={stat.label}><CardContent className="p-6 flex items-center gap-4"><stat.icon size={36} className={stat.color} /><div><p className={`text-3xl font-bold ${stat.color}`}>{stat.value}</p><p className="text-gray-600">{stat.label}</p></div></CardContent></Card>))}</div> ); };
...
export default function DashboardPage() {
  ...
  // === FUNGSI DENGAN PERBAIKAN FINAL DAN DEFINITIF ===
  const fetchAttendanceStatus = useCallback(async (userId: string) => {
    const today = new Date();
    const startOfDay = new Date(today.getFullYear(), today.getMonth(), today.getDate()).toISOString();
    
    // PERBAIKAN: Hapus .maybeSingle() dan proses hasilnya sebagai array
    const { data, error } = await supabase
      ...
      .limit(1); // <-- Hapus .maybeSingle() dari sini

    if (error) {
        // Ini adalah error yang sebenarnya, seperti masalah koneksi atau RLS
    ...
    // Cek jika array 'data' tidak kosong
    if (data && data.length > 0) {
      const latestAttendance = data[0]; // Ambil elemen pertama dari array
        ...
        <main className="max-w-screen-xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex flex-col gap-6 w-full lg:w-1/3">
                    ...
                    <Card><CardHeader><CardTitle className="flex items-center gap-2"><BsCalendarEvent/> Kalender</CardTitle></CardHeader><CardContent><AttendanceCalendar /></CardContent></Card>
                </div>
